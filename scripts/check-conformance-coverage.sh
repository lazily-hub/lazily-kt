...
#
# Pinned at 25 after the collections migration completed #lzktbindpending. In normal work
...
# from the start: `${VAR-25}` substitutes the default only when the variable is
# UNSET, where `${VAR:-25}` would also have swallowed `export
...
EXPECTED_LEDGERED_BLOCKS="${EXPECTED_LEDGERED_BLOCKS-25}"
