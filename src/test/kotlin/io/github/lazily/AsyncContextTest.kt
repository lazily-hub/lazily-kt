...
class AsyncContextTest {
    ...
    @Test
    fun `slot starts empty and resolves on first get_async`() =
        runBlocking {
            val c = ctx()
            val a = c.source(2)
            val sum = c.computedAsync { get(a) + 3 }
    ...
    @Test
    fun `dependency invalidation re-resolves with the new value and discards the stale one`() =
        runBlocking {
            val c = ctx()
            val a = c.source(2)
            val sum = c.computedAsync { get(a) * 10 }
    ...
    @Test
    fun `equal set is a no-op and does not invalidate dependents`() =
        runBlocking {
            val c = ctx()
            val a = c.source(1)
            var runs = 0
            ...
            c.getAsync(s)
            val before = runs
            c.set(a, 1) // equal -> no-op
            assertEquals(before, runs)
            c.dispose()
        }

    @Test
    fun `memo equality guard keeps the published value on an equal recompute`() =
        runBlocking {
            val c = ctx()
            val src = c.source(1)
            val upstream = c.computedAsync { get(src) } // raw source
    ...
    @Test
    fun `concurrent get_async callers share one in-flight compute`() =
        runBlocking {
            val c = ctx()
            val a = c.source(7)
    ...
    @Test
    fun `dropping one waiter does not cancel the shared compute`() =
        runBlocking {
            val c = ctx()
            val a = c.source(1)
            val spawns = AtomicInteger(0)
    ...
    @Test
    fun `error transitions slot to Error and get_async retries on the next call`() =
        runBlocking {
            val c = ctx()
            val mode = c.source("fail")
            var attempt = 0
            val s =
                c.computedAsync {
                    attempt++
                    if (get(mode) == "fail") error("boom")
                    "ok"
                }
            var threw = false
            try {
    ...
    @Test
    fun `async effect reruns on dependency change with cleanup before each body`() =
        runBlocking {
            val c = ctx()
            val a = c.source(0)
    ...
    @Test
    fun `signal eagerly materializes its value after a dependency change`() =
        runBlocking {
            val c = ctx()
            val a = c.source(2)
            val sig = c.signalAsync { get(a) * 3 }
    ...
    @Test
    fun `batch coalesces cell writes into one async rerun`() =
        runBlocking {
            val c = ctx()
            val a = c.source(1)
            val b = c.source(1)
    ...
    @Test
    fun `nested batches flush only at the outermost exit`() =
        runBlocking {
            val c = ctx()
            val a = c.source(0)
            val s = c.computedAsync { get(a) + 1 }
            c.getAsync(s)
            c.batch { outer ->
                outer.set(a, 1)
                outer.batch { inner -> inner.set(a, 2) }
    ...
    @Test
    fun `dispose cancels in-flight compute and runs stored cleanups`() =
        runBlocking {
            val c = ctx()
            val a = c.source(1)
    ...
    @Test
    fun `dynamic dependencies are rediscovered on each recompute`() =
        runBlocking {
            val c = ctx()
            val branch = c.source("left")
            val left = c.source(10)
            val right = c.source(20)
            val s =
                c.computedAsync {
                    if (get(branch) == "left") get(left) else get(right)
                }
            ...
            val before = c.get(s)
            c.set(left, 999) // stale dependency — no longer tracked
            repeat(10) { delay(1) }
